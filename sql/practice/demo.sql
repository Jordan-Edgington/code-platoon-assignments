...
--     name VARCHAR(100) 
--         NOT NULL
--         CHECK(name ~ '^[A-Z][a-z]*( [A-Z][A-Za-z\-]*)*$'),
--     phone VARCHAR(15)
--         CHECK(phone IS NULL OR phone ~ '[5]{3}\-[0-9]{4}$'),
--     email VARCHAR(100)
--         CHECK(email ~ '[a-z0-9._\-]*@{1}[a-z]*[.]{1}com$'),
--     age INT CHECK (age >= 0)
-- );

-- \COPY driver FROM './data/data.csv' WITH CSV HEADER;
-- COMMENT ON TABLE driver IS 'table is a description of the driver and their profile';
